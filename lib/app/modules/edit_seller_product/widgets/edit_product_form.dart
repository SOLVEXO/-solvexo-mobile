  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        if (controller.isEducational) ...[
          const _FieldLabel(label: 'Education Level', required: true),
          const SizedBox(height: 6),
          _EducationLevelField(controller: controller),
          const SizedBox(height: 16),
          Obx(() {
            if (!controller.needsCustomLevel) return const SizedBox.shrink();
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const _FieldLabel(label: 'Custom Level', required: true),
                const SizedBox(height: 2),
                const _FieldHint(hint: 'e.g. "Grade 5", "O-Level", "Hifz"'),
                const SizedBox(height: 6),
...
class _EducationLevelField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(
      () => GestureDetector(
        onTap: () => EditEducationLevelPickerSheet.show(context, controller),
        child: Container(
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Obx(() {
          if (controller.customLevelSuggestions.isEmpty) return const SizedBox.shrink();
          return Padding(
