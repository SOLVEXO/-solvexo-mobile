...
/// Tier-1 education-level picker for the add-product flow — required when
/// the product type is 'educational'. Mirrors [SubcategoryPickerSheet]'s
/// bottom-sheet structure.
class EducationLevelPickerSheet extends StatelessWidget {
  final AddSellerProductController controller;
  const EducationLevelPickerSheet({super.key, required this.controller});

  static void show(BuildContext context, AddSellerProductController controller) {
    Get.bottomSheet(
      EducationLevelPickerSheet(controller: controller),
