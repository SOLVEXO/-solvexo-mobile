...
class RefundRequestRepository {
  ...
  /// GET /api/refund-request/seller/:storeId?page=&limit=
  Future<({List<RefundRequestModel> items, int total})> listForSeller(
    String storeId, {
    int page = 1,
    int limit = 20,
  }) async {
    try {
      final response = await _client.get(
        ApiConstants.refundRequestsForSeller(storeId),
      ...
      return (items: <RefundRequestModel>[], total: 0);
    } on DioException catch (e) {
      debugPrint('❌ RefundRequestRepository.listForSeller DioException: ${e.response?.statusCode}');
      debugPrint('   Response: ${e.response?.data}');
      DioExceptionHandler.handleDioException(e);
      return (items: <RefundRequestModel>[], total: 0);
    } catch (e) {
      debugPrint('❌ RefundRequestRepository.listForSeller error: $e');
      ToastUtil.showToast('Failed to load returns. Please try again.');
      return (items: <RefundRequestModel>[], total: 0);
    }
  }

  /// PATCH /api/refund-request/:id/approve
  Future<bool> approve(String id) async {
    try {
      final response = await _client.patch(
        ApiConstants.approveRefundRequest(id),
        headers: _idempotencyHeader,
        requiresAuth: true,
      );
      ...
      ToastUtil.showToast(
        response.data['message'] as String? ?? 'Failed to approve refund request.',
      );
      return false;
    } on DioException catch (e) {
      debugPrint('❌ RefundRequestRepository.approve DioException: ${e.response?.statusCode}');
      debugPrint('   Response: ${e.response?.data}');
      DioExceptionHandler.handleDioException(e);
      return false;
    } catch (e) {
      debugPrint('❌ RefundRequestRepository.approve error: $e');
      ToastUtil.showToast('Failed to approve refund request.');
      return false;
    }
  }

  /// PATCH /api/refund-request/:id/reject, body: { notes }
  Future<bool> reject(String id, String notes) async {
    try {
      final response = await _client.patch(
        ApiConstants.rejectRefundRequest(id),
        data: {'notes': notes},
        requiresAuth: true,
      );
      ...
      ToastUtil.showToast(
        response.data['message'] as String? ?? 'Failed to reject refund request.',
      );
      return false;
    } on DioException catch (e) {
      debugPrint('❌ RefundRequestRepository.reject DioException: ${e.response?.statusCode}');
      debugPrint('   Response: ${e.response?.data}');
      DioExceptionHandler.handleDioException(e);
      return false;
    } catch (e) {
      debugPrint('❌ RefundRequestRepository.reject error: $e');
      ToastUtil.showToast('Failed to reject refund request.');
      return false;
    }
  }
}
